...
class RandomImagePixelationDataset(Dataset):
    def __init__(self, image_dir, width_range: tuple[int, int], height_range: tuple[int, int], size_range: tuple[int, int], dtype: Optional[type] = None):
        ...
        if self.width_range[1] or self.height_range[0] or self.size_range[0] < 2:
            raise ValueError
        if self.width_range[0] > self.width_range[1] or self.height_range[0] > self.height_range[1] or self.size_range[0] > self.size_range[1]:
            raise ValueError

    def __getitem__(self,    index):
        with Image.open(self.images[index]) as im:
            pic = np.array(im, dtype = self.dtype if self.dtype is not None else None)
        pic = to_grayscale(pic)
